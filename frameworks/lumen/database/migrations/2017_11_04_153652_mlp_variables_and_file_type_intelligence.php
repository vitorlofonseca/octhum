...
class MlpVariablesAndFileTypeIntelligence extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('tbl_intelligence_file_type', function (Blueprint $table) {
            $table->increments('id');
            $table->string('type', 200);
        });

        Schema::table('tbl_intelligence', function (Blueprint $table) {
            $table->integer('id_file_type')->unsigned();
            $table->foreign('id_file_type')->references('id')->on('tbl_intelligence_file_type');
        });
    }
